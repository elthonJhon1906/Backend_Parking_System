...
const ultrasonicRepository = {
  getAllUltrasonic: async (limit, offset) => {
    try {
      const ultrasonics = await sequelize.models.ultrasonic5.findAll({
        limit: limit,
        offset: offset
      });
      return ultrasonics;
  ...
  countUltrasonic: async () => {
    try {
      return await sequelize.models.ultrasonic5.count();
    } catch (error) {
  ...
  createUltrasonic: async (ultrasonicData) => {
    try {
      return await sequelize.models.ultrasonic5.create(ultrasonicData);
    } catch (error) {
